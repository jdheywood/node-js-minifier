	...
	while (defined(myCharA)) {
		...
		if (myCharA == '/') {
			...
			if (defined(myCharB) && myCharB == '/') {
				...
				if (defined(myCharB)) {
					// a is asterisk and b is foreslash
					if (ccFlag) {
						action2(); // the *
						action2(); // the /
		...
		} else if (isAlphanumeric(myCharA)) {
			// keyword, identifiers, numbers
			action1();
			collapseWhitespace();
			if (defined(myCharA) && isWhitespace(myCharA)) {
			...
			action1();
			preserveEndspace();
		} else if (stripDebug && myCharA == ';' && defined(myCharB) && myCharB == ';' && defined(myCharC) && myCharC == ';') {
			...
			// anything else just prints and trailing whitespace discarded
			action1();
			skipWhitespace();
		}
	}
